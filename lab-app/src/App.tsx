...
function App() {
    ...
    const openSuccess = (placement: NotificationPlacement, message: string) => {
        api.success({
            message: ` ${message}`,
            placement,
        });
    };
    const openError = (placement: NotificationPlacement, message: string) => {
        apiError.error({
            message: ` ${message}`,
    ...
    return (
        <div style={{}}>
            <Context.Provider value={contextValue}> {contextHolder} {contextHolderError}</Context.Provider>
            {isLoggedIn ? <DesktopLayout InitialData={initialData} clickLogin={clickLogin} Error={openError} Success={openSuccess} />
                :
                <SignIn clickLogin={clickLogin} Error={openError} Success={openSuccess} />}
            {/* <PdfLayout/> */}
        </div>
    );
}
